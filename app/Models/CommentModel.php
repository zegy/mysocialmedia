...
class CommentModel extends Model
{
    ...
    public function getLikesCom($cid) // get total comment likes. here you can change to get the amount of like by the db view
    {
        $builder = $this->builder('t_like l');
        $queryComLikes = $builder->select('count(*) as qtdlike')
                                 ->join('t_comment c',
                                        'l.like_fk_comment = c.comment_pk')
                                 ->where("c.comment_pk = $cid")
                                 ->get(); 
        return $queryComLikes->getResult()[0]->qtdlike;       
    }

    public function like($cid, $uid) // like a comment. $cid = comment id , $uid = user id who like the comment
    {
        $builder = $this->builder('t_like l');
        ...
        {
            $builder = $this->builder('t_like'); // insert like new and make a new query getting the updated data. insert record conditionally
            $data =
            [
                'like_fk_comment' => $cid,
                'like_fk_user'    => $uid
