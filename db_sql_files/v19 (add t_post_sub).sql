...
DROP TABLE IF EXISTS `t_notif`;
CREATE TABLE `t_notif` (
  `notif_pk` int(11) NOT NULL,
  `notif_to_fk_user` int(11) DEFAULT NULL,
  `notif_from_fk_user` int(11) DEFAULT NULL,
  `notif_fk_post` int(11) DEFAULT NULL,
  `notif_type` varchar(250) COLLATE latin1_general_ci DEFAULT NULL,
  `notif_date_time` datetime DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci;

-- --------------------------------------------------------

--
-- Table structure for table `t_post_sub`
--

DROP TABLE IF EXISTS `t_post_sub`;
CREATE TABLE `t_post_sub` (
  `post_sub_pk` int(11) NOT NULL,
  `post_sub_fk_post` int(11) DEFAULT NULL,
  `post_sub_fk_user` int(11) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci;
...
CREATE TABLE `t_post` (
  ...
  `post_group` varchar(250) DEFAULT NULL,
...
CREATE TABLE `t_user` (
  ...
  `user_id_mix` varchar(250) NOT NULL,
...
INSERT INTO `t_user` (`user_pk`, `user_id_mix`, `user_password`, `user_full_name`, `user_email`, `user_tel`, `user_profile_picture`, `user_regis_date_time`, `user_sex`, `user_bio`, `user_role`, `user_token`) VALUES
(
    1,
    'id_admin',
...
--
ALTER TABLE `t_notif`
  ADD PRIMARY KEY (`notif_pk`),
  ADD KEY `notif_to_fk_user_idx` (`notif_to_fk_user`),
  ADD KEY `notif_from_fk_user_idx` (`notif_from_fk_user`),
  ADD KEY `notif_fk_post_idx` (`notif_fk_post`);

--
-- Indexes for table `t_post_sub`
--
ALTER TABLE `t_post_sub`
  ADD PRIMARY KEY (`post_sub_pk`),
  ADD KEY `post_sub_fk_post_idx` (`post_sub_fk_post`),
  ADD KEY `post_sub_fk_user_idx` (`post_sub_fk_user`);
...
--
ALTER TABLE `t_notif`
  MODIFY `notif_pk` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;

--
-- AUTO_INCREMENT for table `t_notif`
--
ALTER TABLE `t_post_sub`
  MODIFY `post_sub_pk` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;
...
--
-- Constraints for table `t_notif`
--
ALTER TABLE `t_notif`
  ADD CONSTRAINT `notif_to_fk_user` FOREIGN KEY (`notif_to_fk_user`) REFERENCES `t_user` (`user_pk`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `notif_from_fk_user` FOREIGN KEY (`notif_from_fk_user`) REFERENCES `t_user` (`user_pk`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `notif_fk_post` FOREIGN KEY (`notif_fk_post`) REFERENCES `t_post` (`post_pk`) ON DELETE CASCADE ON UPDATE CASCADE;

--
-- Constraints for table `t_post_sub`
--
ALTER TABLE `t_post_sub`
  ADD CONSTRAINT `post_sub_fk_post` FOREIGN KEY (`post_sub_fk_post`) REFERENCES `t_post` (`post_pk`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `post_sub_fk_user` FOREIGN KEY (`post_sub_fk_user`) REFERENCES `t_user` (`user_pk`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `t_comment`
  ...
  ADD CONSTRAINT `comment_fk_user` FOREIGN KEY (`comment_fk_user`) REFERENCES `t_user` (`user_pk`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `t_post`
  ADD CONSTRAINT `post_fk_user` FOREIGN KEY (`post_fk_user`) REFERENCES `t_user` (`user_pk`) ON DELETE CASCADE ON UPDATE CASCADE;
COMMIT;
